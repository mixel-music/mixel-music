...
class LibraryTasks:
    ...
    async def create(path: str, date: datetime = datetime.now()) -> None:
        ...
        track_tags = await ExtractTags(path).extract_tags(tags_table)
        ...
        async with session() as conn:
            async with semaphore_lib:
                await LibraryStore.insert_track(conn, track_tags)
                async with asyncio.TaskGroup() as tg:
                    tg.create_task(LibraryStore.insert_album(conn, album_hash, track_tags))
                    tg.create_task(LibraryStore.insert_artist(conn, artist_hash, track_tags))
            await conn.commit()
    ...
    async def remove(path: str) -> None:
        async with session() as conn:
            async with prevent_block:
                ...
                if album_count == 1: await LibraryStore.delete_album(conn, path)
                ...
                if artist_count == 1: await LibraryStore.delete_artist(conn, path)
