...
class TracksService:
    ...
    async def stream(self, range):  
        ...
        if track_info:
            ...
            async with aiofiles.open(self.path, mode="rb") as music_file:
                await music_file.seek(track_start)
                data = await music_file.read(track_end - track_start + 1)
                headers = {
    ...
    async def create(self):
        self.track_tags = await extract_tags(self.path)
        select_tags = {}
        if not self.track_tags:
            logs.debug("Failed to read tags. Is it a valid file?")
            return False
        try:
            for key, value in self.track_tags:
                if key in list_tags:
                    select_tags[key] = value
                    
                await db.execute(tracks.insert().values(select_tags))
