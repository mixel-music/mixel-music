...
DATABASE_URL = "sqlite:///" + str_path('config', 'database.db', rel=False)
...
tracks = sqlalchemy.Table(
    ...
    sqlalchemy.Column("track", sqlalchemy.String(''), nullable=False),
...
if not get_path('config', 'database.db').exists():
