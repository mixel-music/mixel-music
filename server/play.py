...
async def server_stream(path: str, range: str = Header(None)):
...
"""from fastapi import APIRouter, FastAPI, Request, Response, HTTPException, Header
from . import services

router = APIRouter()

@router.get("/api/stream/{path:path}")
async def server_stream(path: str, range: str = Header(None)):
  root_path = await services.get_root_path()
  print(root_path)
  ext_enable = ['.mp3', '.m4a', '.flac', '.alac', '.wav', '.opus', '.aac']
  ext_import = ''.join(services.Path(path).suffixes)
  file_path = services.Path(f"{root_path}/test/{path}")

  #print(list(Path('.').glob('**/*.mp3')))

  '''
  Check valid ext to prevent directory traversal
  '''
  if ext_import not in ext_enable or ".." in path: raise router.HTTPException(status_code=404)
  if not file_path.is_file(): raise router.HTTPException(status_code=404)
  ...
  with open(file_path, "rb") as file_path:
    ...
    return router.Response(data, status_code=206, headers=headers)"""